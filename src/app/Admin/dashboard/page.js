...
import { Upload, Calendar, MapPin, FileText, Tag, Eye, Edit, Trash2, Plus, Search, ChevronDown, Download, TrendingUp, Users, Ticket, CheckCircle, X, BarChart as BarChartIcon } from 'lucide-react';
...
const groupByMonth = (data, dateKey) => {
    ...
    const grouped = months.map(month => ({ name: month, registrations: 0, sales: 0 }));
...
const filterEvents = (events, filters) => {
    return events.filter(event => {
        ...
        if (filters.searchTerm && !event.eventName?.toLowerCase().includes(filters.searchTerm.toLowerCase())) {
            return false;
        }
...
const calculateAggregatedStats = (filteredEvents, filteredPayments) => {
    ...
    const successfulPaymentsFromPayments = filteredPayments.filter(payment =>
        payment.status === 'completed' || payment.status === 'success'
    ).length;

    // Use the higher value between events and payments for successful payments
    totalSuccessfulPayments = Math.max(totalSuccessfulPayments, successfulPaymentsFromPayments);
...
const generateAggregateParticipationRate = (filteredEvents) => {
    ...
    const participationRate = calculateParticipationRate(totalPlayers, totalMaximumOccupancy);
...
export default function DashBoard() {
    ...
    const getEventStats = (event, userData = [], aggregatedData = null) => {
        ...
        if (event && event.ticketStatus) {
            ...
            return [
                ...
                {
                    label: 'Successful Payment',
                    value: successfulPaymentsCount > 0 ? successfulPaymentsCount : (event.ticketStatus.successfulPayment || 0),
                    Icon: CheckCircle
                },
            ];
        }
    ...
    useEffect(() => {
        if (filteredEvents.length > 0) {
            const aggregated = calculateAggregatedStats(filteredEvents, filteredPayments);
    ...
    useEffect(() => {
        if (showAggregateView && filteredEvents.length > 0) {
            ...
            const aggregateChartData = generateAggregateChartData(filteredEvents, allParticipants);
            setDynamicChartData(aggregateChartData);

            const aggregateParticipationRate = generateAggregateParticipationRate(filteredEvents);
            ...
            const totalPlayers = selectedEvent.ticketStatus?.totalNumberOfPlayers || 0;
    ...
    }, [showAggregateView, filteredEvents, selectedEvent, userdata, allParticipants]);
    ...
    const handleFilterChange = (filterName, value) => {
        setFilters(prev => {
            ...
            else if ((filterName === 'dateFrom' || filterName === 'dateTo') && value) {
                newFilters.month = '';
            }
    ...
    const chartDataToUse = dynamicChartData.length > 0 ? dynamicChartData : defaultChartData;
    const participationRateToUse = participationRate.length > 0 ? participationRate : defaultParticipationRate;
            ...
            <div className="max-w-7xl mx-auto">
                <div className="bg-white rounded-2xl shadow-lg p-6 mb-8">
                    ...
                    <div className="flex gap-4 ">
                        ...
                        {/* Event Category */}
                        <div className="flex-1">
                            <label className="block text-sm font-semibold text-slate-700 mb-3">Event Category</label>
                            <div className="relative">
                        ...
                        <div className="flex-1 flex items-end">
                            <div className="w-full flex justify-center">
                                <button
                                    ...
                                    disabled={!filters.dateFrom && !filters.dateTo && !filters.month && !filters.category && !filters.status && !searchTerm}
                                    className={`flex items-center gap-2 px-4 py-3 text-sm border rounded-lg transition-all ${(filters.dateFrom || filters.dateTo || filters.month || filters.category || filters.status || searchTerm)
                    ...
                    <div className="pt-5">
                        <div className="flex justify-between items-center mb-4">
                            <h1 className="text-black font-bold text-2xl">
                                Events List
                                {(filters.dateFrom || filters.dateTo || filters.month || filters.category || filters.status || searchTerm) && (
                                    ...
                                    <tbody>
                                        {filteredEvents.length > 0 ? (
                                            filteredEvents.map((event, idx) => (
                                                <tr
                                                    key={event.id || idx}
                                                    className={`border-t border-primary/10 cursor-pointer transition-all hover:bg-primary/5 hover:shadow-md ${selectedEvent?.id === event.id
                                                        ? 'bg-primary/20 ring-2 ring-primary/30 border-l-4 border-l-primary shadow-lg'
                                                        : 'bg-white'
                                                        } ${showAggregateView ? 'opacity-50' : ''}`}
                                                    onClick={() => !showAggregateView && handleEventClick(event)}
                                                >
                                                    ...
                                                    <td className="px-6 py-4">{event.category || 'General'}</td>
                                                    <td className="px-6 py-4">
                                                        <span className={`px-4 py-1 rounded-full text-xs font-semibold ${getStatusClasses(event.status)}`}>
                            ...
                            <div className="flex flex-col items-center">
                                    ...
                                    <PieChart>
                                        ...
                                        >
                                            {participationRateToUse.map((entry, index) => (
                                                <Cell
                                                    key={`cell-${index}`}
                                                    fill={index === 0 ? 'var(--primary)' : 'rgb(229 231 235)'}
                                                />
                                            ))}
                                        </Pie>
                                        <Tooltip />
                                ...
                                {selectedEvent && (
                                    <div className="mt-2 text-center text-sm text-slate-600">
                                        <p>Maximum Occupancy: {selectedEvent.ticketStatus?.maximumOccupancy || 0}</p>
                                        <p>Total Players: {selectedEvent.ticketStatus?.totalNumberOfPlayers || 0}</p>
                                        <p className="font-semibold">Participation Rate: {participationRateToUse[0]?.value || 0}%</p>
                                    </div>
                                )}
                                {showAggregateView && filteredEvents.length > 0 && (
                                    <div className="mt-2 text-center text-sm text-slate-600">
                                        <p>Total Maximum Occupancy: {aggregatedStats?.maximumOccupancy || 0}</p>
                                        <p>Total Players: {aggregatedStats?.totalPlayers || 0}</p>
                                        <p className="font-semibold">Aggregate Participation Rate: {participationRateToUse[0]?.value || 0}%</p>
                                    </div>
                                )}
                            </div>
                        </div>
                ...
                <div className="bg-white rounded-2xl shadow-lg p-6 mb-8 space-y-8">
                    ...
                    <div>
                        <div className="flex justify-between items-center mb-3">
                            <h2 className="text-slate-900 font-semibold">Event Performance</h2>
                                ...
                                <tbody>
                                    {eventPerformance.length > 0 ? (
                                        eventPerformance.map((row, idx) => (
                                            <tr key={row.eventId || idx} className="border-t border-primary/10 bg-white">
